...
/**
 * \file htdocs/fourn/commande/release.php
 * \ingroup commande
...
require_once DOL_DOCUMENT_ROOT . '/custom/sorderplus/class/fournisseur.commande.release.class.php';
...
{
	$error=0;
	$supplierorderdispatch = new CommandeFournisseurRelease($db);
...
if ($id > 0 || ! empty($ref)) {
	...
	if ($object->statut == 3 || $object->statut == 4 || $object->statut == 5) {
		...
		print '<form method="POST" action="release.php?id=' . $object->id . '">';
		print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
		print '<input type="hidden" name="action" value="release">';
		...
		if ($nbproduct)
		{
			...
			print '<br><input type="submit" class="button" value="' . 'Libérer' . '"';
